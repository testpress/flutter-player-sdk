...
enum class PROVIDER(val raw: Int) {
  TPSTREAMS(0),
  TESTPRESS(1);

  companion object {
    fun ofRaw(raw: Int): PROVIDER? {
      return values().firstOrNull { it.raw == raw }
    }
  }
}
private open class NativeSDKApiPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      129.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
          PROVIDER.ofRaw(it.toInt())
        }
      }
...
interface NativeSDKApi {
  ...
  companion object {
    ...
    val codec: MessageCodec<Any?> by lazy {
      NativeSDKApiPigeonCodec()
    }
    /** Sets up an instance of `NativeSDKApi` to handle messages through the `binaryMessenger`. */
    @JvmOverloads
    fun setUp(binaryMessenger: BinaryMessenger, api: NativeSDKApi?, messageChannelSuffix: String = "") {
      val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.tpstreams_player_sdk.NativeSDKApi.initialize$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            val allowFallbackToL3Arg = args[3] as Boolean
